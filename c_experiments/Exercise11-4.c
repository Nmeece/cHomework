...
// Argument: Pointer to list item
void removeEntry (struct entry *refEntry) {
   // assert(!(refEntry->next && refEntry->previous));

    // if refEntry->next != NULL, Then the item pointed to by refEntry->next needs it's ->previous changed to refEntry->previous
    // otherwise, the item pointed to by refEntry->previous needs its ->next set to refEntry->next

    if ((refEntry->next != NULL) && (refEntry->previous != NULL)) {
        refEntry->next->previous = refEntry->previous;
        refEntry->previous->next = refEntry->next;
    }
    if ((refEntry->next) == NULL) {
        refEntry->previous->next = NULL;
    }
    else {
        refEntry->next->previous == NULL;
    }
}
